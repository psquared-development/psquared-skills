#!/usr/bin/env node
/**
 * build.mjs — Render a psquared security audit report PDF from the config
 * that scripts/prepare.py writes.
 *
 * Usage:
 ...
 * Run scripts/prepare.py first; see examples/report-example.json.
 */
...
<head>
  <meta charset="utf-8" />
  <base href="${baseHref}" />
  <title>${escapeHtml(cfg.doc?.title || "Security Audit")}</title>
...
<body>
  ...
  const tmp = await mkdtemp(path.join(tmpdir(), "psquared-security-report-"));
  ...
  try {
    ...
    const brandText = `${cfg.provider?.name || "psquared GmbH"} · Security Audit`;
