#!/usr/bin/env bash
# Idempotent setup script for the create-security-report skill.
...
echo "==> create-security-report setup"
...
echo ""
echo "Setup complete."
echo "Try: python3 scripts/prepare.py --run <run-dir> --report examples/report-example.json --out /tmp/report-config.json && node build.mjs /tmp/report-config.json output.pdf"
